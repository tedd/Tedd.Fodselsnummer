...
public static class FodselsnummerValidator
{
    ...
    {
        // 10 digits (starts with 0, so min 1_000_000_000) or 11 digits (max 99_999_999_999)
    ...
    public static FodselsnummerResult Validate(string number)
    {
        ...
        long parsedNumber;
        if (!long.TryParse(number, NumberStyles.None, CultureInfo.InvariantCulture, out parsedNumber))
            return FodselsnummerResult.FromError(2);
